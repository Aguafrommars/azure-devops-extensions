...
if "%APPVEYOR_REPO_BRANCH%" EQU "master" (
    tfx extension create --override "{\"version\": \"%SemVer%.%GitVersion_BuildMetaData%\"}" --overrides-file ..\override.json --output-path dist
) else (
    tfx extension create --override "{\"version\": \"%SemVer%.%GitVersion_BuildMetaData%\"}" --overrides-file --output-path dist
)
