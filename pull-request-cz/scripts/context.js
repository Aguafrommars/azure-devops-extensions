...
function registerContribution(context) {
  ...
  if (completionOptions) {
    ...
    document.getElementById("squash").checked =
      completionOptions.squashMerge || false;
    document.getElementById("delete-source-branch").checked =
      completionOptions.deleteSourceBranch || true;
    document.getElementById("complete-workitems").checked =
  ...
  VSS.require(
    ...
    function (VSS_Service, TFS_Wit_WebApi) {
      // Get the REST client
      const gitClient = VSS_Service.getCollectionClient(
        TFS_Wit_WebApi.GitHttpClient2_2,
      );

      gitClient.getPullRequestWorkItemRefs =
        gitClient.getPullRequestWorkItemRefs ||
        gitClient.getPullRequestWorkItems;
      gitClient
        .getPullRequestWorkItemRefs(pr.repositoryId, pr.pullRequestId)
        .then(
          function (items) {
  ...
  VSS.getAccessToken().then(function (token) {
    ...
    xhr.open(
      "GET",
      `${context.collection.uri}${pr.projectGuid}/_apis/policy/evaluations?artifactId=${encodeURIComponent(`vstfs:///CodeReview/CodeReviewId/${pr.projectGuid}/${pr.codeReviewId}`)}`,
    );
    ...
    xhr.onload = function () {
      if (xhr.status === 200) {
        ...
        for (const policy of policies) {
          ...
          if (configuration.isEnabled) {
            ...
            if (
              settings.allowNoFastForward !== undefined ||
              settings.allowSquash !== undefined ||
              settings.useSquashMerge !== undefined
            ) {
              const squash = document.getElementById("squash");
              saveSquash = false;
              squash.disabled = true;
              squash.checked = settings.allowNoFastForward
                ? false
                : settings.allowSquash || settings.useSquashMerge;
            } else if (
              configuration.isBlocking &&
      ...
      VSS.getService(VSS.ServiceIds.ExtensionData).then(function (dataService) {
        ...
        dataService
          .getValue("preferences", { scopeType: "User" })
          .then(function (value) {
            if (!value) {
              return;
            }
  ...
  function createTypeCombo() {
    VSS.require(
      ["VSS/Controls", "VSS/Controls/Combos"],
  ...
  const registrationForm = (function () {
    ...
    $("#auto-complete").on("click", function () {
      ...
      VSS.require(
        ...
        function (VSS_Service, TFS_Wit_WebApi) {
          ...
          gitClient
            .updatePullRequest(patch, pr.repositoryId, pr.pullRequestId)
            .then(
              function () {
                notify();
              },
        ...
        function (VSS_Service, TFS_Wit_WebApi) {
          ...
          const patch = {
            ...
            lastMergeSourceCommit:
              ...
              pr.pullRequestCard.gitPullRequest.lastMergeSourceCommit,
          };
    ...
    function getCompletionOptions() {
      ...
      VSS.getService(VSS.ServiceIds.ExtensionData).then(function (dataService) {
        ...
        dataService
          .setValue("preferences", preferences, { scopeType: "User" })
          .then(function (value) {});
      });
    ...
    document.getElementById("delete-source-branch-label").textContent =
      `Delete ${pr.sourceFriendlyName} after merging`;
